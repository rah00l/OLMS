...
LoginCheck.prototype = {
  ...
  initialize: function() {
    ...
    if (!user_name) {  
      ...
      $('login').update(dummy.innerHTML);  /* FF doesn't support outerHTML */
    } else {
      var dummy = document.createElement('span');
      var link = document.createElement('a');
      dummy.appendChild(link);
...
function test_cc_number() {
  $('ccnumber').value = "4111111111111111";
  $('cvv').value = "123";
  $('ccexp').value = "1010";
}

function validateCreditCardForm(submit_button, thank_you_button) {
  var fields = { ccnumber  : 'credit card number',
...
function notInterestedInKindle() {
  $('kindle_banner').fade();
  new Ajax.Request('/users/kindle_no_thanks', {'method':'POST'})
}

function setIntroHeight() {
  var height = $('book-detail').getHeight() - $('head-wrapper').getHeight() - 25;
  if (/MSIE [456]/.test(navigator.userAgent) && !window.opera ) {
